...
module csrng_ctr_drbg_gen import csrng_pkg::*; (
  ...
  localparam int GenreqFifoWidth = KeyLen+BlkLen+CntrLen+1+SeedLen+1+InstIdWidth+CmdWidth;
  ...
  localparam int RCStageFifoWidth = KeyLen+BlkLen+BlkLen+CntrLen+1+1+InstIdWidth+CmdWidth;
  localparam int GenbitsFifoDepth = 1;
  localparam int GenbitsFifoWidth = 1+BlkLen+KeyLen+BlkLen+CntrLen+InstIdWidth+CmdWidth;
...
//
  ...
  assign sfifo_genreq_wvld = ctr_drbg_gen_enable_i && ctr_drbg_gen_req_i;
  ...
  for (genvar i = 0; i < NumApps; i = i+1) begin : gen_adata
    assign capt_adata[i] = (sfifo_adstage_wvld && (genreq_id == i));
