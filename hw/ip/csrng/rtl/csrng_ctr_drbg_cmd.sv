...
module csrng_ctr_drbg_cmd import csrng_pkg::*; (
  ...
  localparam int RCStageFifoWidth = KeyLen+BlkLen+InstIdWidth+CntrLen+1+SeedLen+1+CmdWidth;
  localparam int KeyVRCFifoDepth = 1;
  localparam int KeyVRCFifoWidth = KeyLen+BlkLen+CntrLen+1+SeedLen+1+InstIdWidth+CmdWidth;
  ...
  assign prep_rc =
         ...
         (cmdreq_data.cmd == GEN) ? cmdreq_data.rc_cnt :
         (cmdreq_data.cmd == UPD) ? cmdreq_data.rc_cnt :
         '0;
  ...
  assign sfifo_keyvrc_wdata = (rcstage_ccmd == UNI) ?
         {{(KeyLen+BlkLen+CntrLen+1+SeedLen){1'b0}},rcstage_glast,cmd_upd_rsp_data_i.inst_id,cmd_upd_rsp_data_i.cmd} :
         gen_adata_null_q ?
