...
module top_earlgrey_cw340 #(
  ...
  logic usb_rx_d;
  logic usb_rx_enable;
  assign usb_dp_pullup_en_o = usb_dp_pullup_en;
  assign usb_rx_d           = usb_rx_d_i;
  assign usb_rx_enable_o    = usb_rx_enable;
  ...
  // reset used for test mode (sourced by the AST)
  logic scan_rst_n;
  ...
  ast u_ast (
    ...
    .clk_src_usb_o         ( ast_base_clks_o.clk_usb ),
  ...
  if (BkdrLoaderEn) begin : gen_bkdr
    ...
    always_comb begin : proc_conn_bkdr
      ...
      jtag_req_i.tck      = mio_in_i[TckPadIdx];
      jtag_req_i.tms      = mio_in_i[TmsPadIdx];
      jtag_req_i.trst_n   = mio_in_i[TrstNPadIdx];
      jtag_req_i.tdi      = mio_in_i[TdiPadIdx];
      mio_out_o[TdoPadIdx]  = jtag_rsp_o.tdo;
      mio_oe_o[TdoPadIdx]   = jtag_rsp_o.tdo_oe;
      mio_attr_o[TdoPadIdx] = '0;
    ...
    assign tap_strap0 = mio_in_i[Tap0PadIdx];
    assign tap_strap1 = mio_in_i[Tap1PadIdx];
  ...
  earlgrey_pd_aon #(
    ...
    .clk_main_i(ast_base_clks_o.clk_sys),
    .clk_io_i  (ast_base_clks_o.clk_io ),
    .clk_usb_i (ast_base_clks_o.clk_usb),
    .clk_aon_i (ast_base_clks_o.clk_aon),
