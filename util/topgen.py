    ...
    if not args.no_top or args.top_only:
        ...
        for target in completecfg["targets"]:
            ...
            render_template(top_template_path / f"top_{topname}.sv.tpl",
                            out_path / f"rtl/autogen/top_{topname}_{target_name}.sv",
                            gencmd=gencmd_sv,
                            target=target)
            # Per-target chip-level. Contains only padring, physical cells and the top
            # wrapper above.
            render_template(top_template_path / "chiplevel.sv.tpl",
                            out_path / f"rtl/autogen/chip_{topname}_{target_name}.sv",
                            gencmd=gencmd_sv,
                            target=target)
