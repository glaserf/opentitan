...
- We might want to start with enforcing `domain` and `domain_secondary` not specifying the same PD. Perspectively, it might however be desirable to have the possibility to instantiate splittable IPs in a single PD. In this case, the intra-IP / inter-partition connections must not leave the power domain wrapper, and `topgen/intermodule.py` need to create connections and signals slightly differently (see [`topgen` discussion](#topgen-and-template-implications)).
