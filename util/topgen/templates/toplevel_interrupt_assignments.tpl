...
<%
  ...
  if intr["width"] > 1:
    ...
    intr_comment = f"// ID {base}"
  if intr["phys_pd"] == phys_pd:
    intr_sig = f"intr_{intr['name']}"
  else:
    count_pd[intr["phys_pd"]] -= intr["width"]
    idx_l = count_pd[intr["phys_pd"]]
    plic_str = "_" + plic if len(top["plic_info"]) > 1 else ""
    intr_sig = f"intr_vector{plic_str}_pd_{intr['phys_pd']}_i[{idx_l}]"
...
% else:
<%
  intrs = [i for i in top["interrupt"] if i["plic"] == plic and i["phys_pd"] == phys_pd]
  if len(intrs) == 0:
    continue
  plic_str = "_" + plic if len(top["plic_info"]) > 1 else ""
%>\
  // Interrupt vector to PLIC ${plic} in power domain ${info["phys_pd"]}
