...
def commit_alert_connections(top: ConfigT,
    ...
    # Construct an info dict
    alert_handlers = lib.find_modules(top["module"], "alert_handler")
    alert_handler_info = defaultdict()
    for ah in alert_handlers:
        phys_pd = ah.get("phys_pd", top["power"]["physical"][0])
        ah_info = {"phys_pd": phys_pd, "count_tot": 0, "count_pd": {}, "connect_pd": {}}
        for pd in top["power"]["physical"]:
    ...
    for module in top["module"]:
        ...
        if block.alerts:
            ...
            m_phys_pd = module.get("phys_pd", top["power"]["physical"][0])
            a_phys_pd = alert_handler_info[handler]["phys_pd"]
            ...
            if outgoing:
                ...
                len_pd = alert_handler_info[handler]["count_pd"][m_phys_pd]
                if m_phys_pd != a_phys_pd:
                    for i in range(w):
                        conn_info = {"src_pd": m_phys_pd, "idx": len_pd+i}
                ...
                if m_phys_pd != a_phys_pd:
                    alert_tx += "_o"
                    alert_rx += "_i"
                    lo = alert_handler_info[handler]["count_pd"][m_phys_pd]
                else:
                ...
                alert_handler_info[handler]["count_pd"][m_phys_pd] += w
    ...
    for name, info in alert_handler_info.items():
        ...
        for pd in top["power"]["physical"]:
            width = info["count_pd"][pd]
            if pd != info["phys_pd"] and width > 0:
                sig_names = (signals[0] + f"_pd_{pd}", signals[1] + f"_pd_{pd}")
                sig_tx = proto.copy()
                sig_rx = proto.copy()
