...
def commit_alert_connections(top: ConfigT,
    ...
    for module in top["module"]:
        ...
        if block.alerts:
            ...
            m_phys_pd = module.get("domain", top["power"]["default"])
            a_phys_pd = alert_handler_info[handler]["domain"]
...
def amend_pinmux_io(top: ConfigT,
    ...
    for m in top["module"]:
        ...
        block = name_to_block.get(m['type'])
        if block is None:
            # TODO figure out what the devil is going on here;
            # name_to_block seems to miss a lot of IP types
            continue
